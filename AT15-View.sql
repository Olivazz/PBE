...
CREATE VIEW vw_cliente_detalhes AS SELECT nome_cliente, email_cliente, telefone_cliente, cidade FROM tb_cliente;
...
CREATE VIEW vw_produtos_estoque AS SELECT nome_produto, categoria, estoque FROM tb_produto WHERE estoque > 100;
...
CREATE VIEW vw_categorias AS SELECT categoria, count(estoque) AS estoque_categoria FROM tb_produto GROUP BY categoria;
...
SELECT nome_produto, lucro FROM vw_produtos_precos WHERE lucro = '5.00';

-- 10. Usando a view "vw_categorias", selecione as categorias com mais de 10 produtos cadastrados.
SELECT categoria, estoque_categoria FROM vw_categorias WHERE estoque_categoria > 10;
...
CREATE VIEW vw_clientes_aniversariantes AS SELECT nome_cliente, telefone_cliente,
MONTH(data_nascimento) AS mes_nascimento 
FROM tb_cliente WHERE MONTH(data_nascimento) BETWEEN 1 AND 6;
...
CREATE VIEW vw_produtos_lucro AS SELECT nome_produto, categoria, estoque, preco_venda, preco_compra,
preco_venda - preco_compra AS lucro_bruto FROM tb_produto
WHERE estoque > 50;
SELECT * FROM vw_produtos_lucro;

/* 13. Crie uma VIEW chamada "vw_clientes_mais_lucrativos" que exibe o nome do cliente, telefone, cidade, estado e a soma do lucro bruto de todos os produtos com estoque menor que 50 vendidos
 para clientes que moram em estados do Sudeste (SP, RJ, MG, ES) */
CREATE VIEW vw_clientes_mais_lucrativos AS SELECT nome_cliente, telefone_cliente, cidade, estado FROM tb_cliente INNER JOIN tb_produtos
WHERE estoque < 50;

/* 14. Usando a view "vw_clientes_aniversariantes", selecione os nomes dos clientes que fazem aniversário em um mês ímpar e ordene pelo mês de nascimento. Utilize uma condição para exibir
 o mês por extenso (ex: 'Janeiro', 'Março', etc.) */


/* 15. Usando as views "vw_produtos_lucro" e "vw_clientes_mais_lucrativos", selecione os nomes dos clientes e o lucro total de produtos da categoria 'Alimentos' vendidos para esses clientes,
 mas apenas para aqueles que geraram lucro acima de 100. Ordene pelo lucro em ordem decrescente */
